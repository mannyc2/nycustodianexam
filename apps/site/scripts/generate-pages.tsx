...
const buildPages = ({
  ...
  <main class="page-shell" id="main-content" tabindex="-1">
    ...
    ${comparableFamilies.length === 0 ? "" : `<section class="atlas-related-section" aria-labelledby="atlas-families-heading"><header class="atlas-section-heading"><h2 id="atlas-families-heading">Explore tool families</h2><p>Compare uses and recognition cues across a whole family.</p></header><ul class="atlas-family-grid">${comparableFamilies.map(([family, tools]) => `<li><a class="atlas-family-card" href="/atlas/family/${slugify(family)}/"><span class="atlas-related-card-copy"><strong>${escapeHtml(capitalize(family))}</strong><span>${tools.length} tools</span></span><span aria-hidden="true">→</span></a></li>`).join("")}</ul></section>`}
  </main>
  <script type="module" src="/src/static-browser.ts"></script>`
  })
