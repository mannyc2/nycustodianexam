...
test("nonvisual question presentation survives commitment, reload, and export/import", async ({ page }) => {
  const feedbackReads: string[] = []
  page.on("request", request => {
    if (request.method() === "GET" && request.url().includes("/questions/v2/q091.postcommit.json")) feedbackReads.push(request.url())
  })
  await page.goto("/practice/session/launch-v1/question/91/")
  const toggle = page.getByRole("button", { name: "Use nonvisual version", exact: true })
  await expect(toggle).toBeEnabled()
  await toggle.click()
  await expect(page.getByRole("button", { name: "Use illustrated version", exact: true })).toBeFocused()
  ...
  await expect(page.getByText("Question 91 of 91 · Nonvisual version", { exact: true })).toBeVisible()
  await page.getByRole("radio", { name: "Pipe wrench", exact: true }).check()
  await page.getByRole("button", { name: "Use illustrated version", exact: true }).click()
  await expect(toggle).toBeFocused()
  await expect(page.locator(".question-nonvisual-body")).toHaveCount(0)
  await expect(page.locator(".question-illustration")).toHaveCount(1)
  await expect(page.getByRole("radio", { name: "Pipe wrench", exact: true })).toBeChecked()
  await toggle.click()
  await expect(page.getByRole("button", { name: "Use illustrated version", exact: true })).toBeFocused()
  await expect(page.getByRole("radio", { name: "Pipe wrench", exact: true })).toBeChecked()
  ...
  await expect(page.getByRole("button", { name: "Use illustrated version", exact: true })).toBeDisabled()
  await page.reload()
  await expect(page.getByRole("heading", { name: "Which tool matches these observable features?", exact: true })).toBeVisible()
  await expect(page.getByText("Answered using the nonvisual version.", { exact: true })).toBeVisible()
  await roundTripSavedRecord(page, "attempts")
