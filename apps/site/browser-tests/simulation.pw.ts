...
test("builds a deterministic-capacity simulation, restores edits, and commits before reveal", async ({
  ...
  await expect(page.locator('.simulation-navigator a[aria-current="step"]')).toBeFocused()
...
test("nonvisual question presentation persists through simulation reload and final results", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "Use nonvisual version", exact: true })).toBeFocused()
...
test("active Simulation and results retain saved answers through lifecycle cleanup", async ({ page }) => {
  const errors: string[] = []
  ...
  await page.getByRole("button", { name: "Submit final answers", exact: true }).click()
  await expect(page).toHaveURL(/\/results\/$/)
  const results = page.locator("[data-simulation-results]")
  await expect(results.locator("li.reference-card").first()).toBeVisible()
  const resultCount = await results.locator("li.reference-card").count()
  const resultsPath = page.url()
  await page.evaluate(() => window.dispatchEvent(new PageTransitionEvent("pagehide", { persisted: true })))
  ...
  await page.reload()
  await expect(page).toHaveURL(resultsPath)
  await expect(results.locator("li.reference-card")).toHaveCount(resultCount)
