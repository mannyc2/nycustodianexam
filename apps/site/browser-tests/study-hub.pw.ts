...
test("a generated practice answer appears in Study and can be durably finished in Review", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: "Open saved feedback", exact: true })).toHaveAttribute("href", sessionPath!)
  await page.getByRole("link", { name: "Open saved feedback", exact: true }).click()
  await expect(page.locator(".feedback-rationales")).toBeVisible()
  await page.goto("/review/")
  await page.getByRole("button", { name: "Finish review", exact: true }).click()
  await expect(page.getByRole("heading", { name: "No review items are due", exact: true })).toBeFocused()
  await expect(page.getByRole("region", { name: "Review history" })).toContainText("Review finished")
  await page.reload()
  await expect(page.getByRole("heading", { name: "No review items are due", exact: true })).toBeVisible()
