...
| Illustration production pipeline, 2026-08-17 | **RECOVERED-DURABLE** | `illustration/PIPELINE_SPEC.md` | Normalized production mechanics restored. Historical inventory cannot override current taxonomy. |
| Historical tool QA matrix | **RECOVERED-DURABLE / STALE-INVENTORY** | Integrity recorded in `illustration/RECOVERED_ASSET_MANIFEST.md` | 120-row snapshot. Reconcile/regenerate from current TAXONOMY before generation. |
| Historical hazard cue/decoy QA matrix | **RECOVERED-DURABLE / STALE-INVENTORY** | Integrity recorded in manifest | 33-row snapshot. Reconcile before use. |
...
| SVG tool examples and matched confusion panels | **RECOVERED-PROTOTYPE** | `illustration/examples/` | Source SVGs are useful prototypes; not automatically production-approved. Preview PNG/contact-sheet derivatives are intentionally not a separate source of truth. |
...
| Cloudflare deployment direction | **CURRENT PREFERENCE, NOT LOCKED** | `product/ARCHITECTURE_CONSTRAINTS.md` | Requires current primary-source architecture research before deployment config is frozen. |
| Dedicated modern frameworkless/Cloudflare architecture research | **NOT YET DONE** | remains open | Do this after corpus reconciliation, not before. |

## Exact recovered artifact integrity
...
4. **Historical inventory is not current scope.** Old QA matrices cannot delete or downgrade newer taxonomy entries.
5. **Unknowns remain unknown.** Recovery does not resolve official item counts, weights, conversion, form identity, review logistics, or other open facts.
6. **Security boundary survives recovery.** No secure/recalled/reconstructed exam item content is imported.
7. **Later maintainer decisions are explicit.** Plain HTML/CSS/TypeScript is now a current implementation constraint; Cloudflare remains preferred but unverified at configuration level.
