...
const p1=await newPage('http://127.0.0.1:4173/probe.html');
...
const p2=await newPage('http://127.0.0.1:4173/probe.html');
...
const lock1=p1.c.eval(`navigator.locks.request('r24-pack-install',async()=>{globalThis.__lock1Start=performance.timeOrigin+performance.now();await window.R24.sleep(300);globalThis.__lock1End=performance.timeOrigin+performance.now();return {start:globalThis.__lock1Start,end:globalThis.__lock1End}})`);
await sleep(50);
const lock2=p2.c.eval(`navigator.locks.request('r24-pack-install',async()=>({acquired:performance.timeOrigin+performance.now()}))`);
