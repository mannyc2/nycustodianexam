...
def exercise(page: Any, base: str, name: str) -> dict[str, Any]:
    ...
    page.locator("#flag-button").click()
    # persistSession is fire-and-forget; allow the IndexedDB write to settle
    # before navigating (immediate navigation loses the un-started write).
    page.wait_for_timeout(300)
    # reload without ?reset=1 so the restoration path is actually exercised
...
def main() -> None:
    ...
    try:
        with sync_playwright() as p:
            browser = p.chromium.launch(executable_path="/opt/pw-browsers/chromium-1194/chrome-linux/chrome", headless=True, args=["--no-sandbox"])
