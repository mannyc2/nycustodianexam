...
| Custom question setup | `practice-custom` — `/practice/custom/` | Practice → Choose your own topics and length | `noindex`; safe bootstrap + question builder island | Available in downloaded study copies; no answer data in setup | Explicit topic/length choices; invalid capacity blocks start; legacy builder fragment redirects here | M2 |
...
Receipt-bound previous inventories may generate noindex `review-player` and
`hazard-player` documents under `/history/{releaseId}-v{packVersion}/`, followed by the original
practice, review or hazard path. These are ordinary Static Assets documents, not dynamic
