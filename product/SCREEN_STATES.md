...
|---|---|---|---|---|
| 1. `home` | `ready`, `offline-stale`; static navigation reflects only publication-safe availability | Read exam information or continue to Practice; no island boot may hide the public Home document | Normal document entry; no dashboard state in the URL | Cached public overview/navigation remains fully useful |
| 2. `exam-selector` | `ready` ↔ filtered `empty`; opening a record shows its reference panel and tabs | Clear search or open a profile; opening a record focuses its panel; result-count announcement is polite | The `record` query identifies the displayed reference; browsing does not write a study preference; profile links use document navigation | Cached registry may be `offline-stale`; absent registry is `offline-unavailable`, not empty |
...
| 4. `profile` | `ready` with fact freshness states; `superseded` or `retired`; reference browsing has no selection operation | Fact conflict links to both sources; explicit reference and practice destinations remain available | Administration URL is immutable and reloadable; reading facts never changes an active session pin or study scope | Cached page stays readable as `offline-stale`; external links and update checks are disabled truthfully |
| 5. `study-hub` | Static statewide-series practice starts; saved activity is `loading`, `ready`, or `unavailable`; missing compatible inventory is `empty` | Unavailable saved activity offers retry and focuses its recovery heading; it is never presented as a new learner; available set links remain explicit | Practice links navigate to generated session URLs; saved activity is restored from local events, not an exam preference or URL | Cached guidance works; practice requires compatible available content; no global exam selection is required |
