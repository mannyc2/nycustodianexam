...
const output = fileURLToPath(new URL('./settings-export-screenshots/', import.meta.url));
...
try {
  for (const width of [608, 384]) {
    ...
    const panel = page.locator('#export-local-data');
    await panel.getByText('What the file includes', { exact: true }).click();
    await expect(panel.getByText(/never uploaded/)).toBeVisible();
    const file = `contents-${width}.png`;
    await panel.screenshot({ path: output + file, style: '.site-header-inner > .nav-primary, .site-header-inner > .nav-utility { opacity: 0; }' });
    captures.push({ file, width, draftsIncluded: false });
    await context.close();
  }
