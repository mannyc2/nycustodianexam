...
const output = path.join(reviewRoot, 'current-screenshots');
...
async function capture(page, name, state, width, fullPage = true) {
  ...
  captures.push({ file, route: new URL(page.url()).pathname, state, width, height: 900, fullPage });
  console.log(file);
}
...
try {
  for (const width of [1280, 390]) {
    ...
    for (const [name, route, ready] of [
      ...
      ['offline-first-visit', '/offline/', 'Nothing downloaded yet'],
      ['simulation-setup', '/simulations/', null],
      ['print-setup', '/print/', null],
