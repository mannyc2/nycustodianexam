...
Source inspection at `6bc18eae05949b531ac13afbab03127bb67b4179`, against `product/COMPONENT_ARCHITECTURE.md` sections 2–3 and 6–12. This is an incomplete-work ledger, not acceptance from passing tests.
...
| --- | --- | --- |
| Settings | `settings/react/settings.tsx` owns workflow state and calls runtime for preferences, export/import and deletion | Move workflow orchestration to a renderer-neutral controller; provider owns subscription/focus, leaves consume state/actions/meta. Preserve preview/confirmation and durable transfer semantics. |
| Offline | `offline-packs/react/pack-manager.tsx` owns lifecycle state and executes reconciliation/download/removal effects from React | Controller/provider boundary for download, storage recovery and removal impact. Preserve historical data and no optimistic activation. |
...
| Review | `review/react/review-queue.tsx` directly subscribes and dispatches retry/rebuild/acknowledge | Provider boundary and explicit queue pieces, preserving overlapping filters and confirmation/focus. |
