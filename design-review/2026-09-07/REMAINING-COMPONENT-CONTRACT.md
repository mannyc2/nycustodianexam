...
## Shared static source-line citations

Profile facts and the generator's source-line lists now use `apps/site/scripts/source-line-citations.tsx`. The shared renderer retains source titles, canonical internal links, exact locators and excerpts, and fails generation when referenced evidence is missing. It remains static semantic HTML without a client island. This closes duplicated source-line rendering; the full SourceCitation tier/publisher/date/online-state/supported-claim composition remains open.

Validation: site TypeScript passed; all 17 static-generation tests passed, including hostile-text escaping and missing-evidence failures. Production build and artifact verification passed: 1,352 documents, 224 item-scoped artifacts, 291 byte-identical delivery assets, and 61 safe shell URLs. Logs: `/tmp/nyc-shared-citations-tests.log` and `/tmp/nyc-shared-citations-build.log`. No fresh browser or visual fidelity claim is made for this extraction.

## Source detail citation metadata

Source detail pages now compose static `SourceCitation.Metadata`, `OnlineLink`, and `RetainedLines` pieces. They expose the catalog's evidence tier/version alongside publisher, locator and supported scope; retained catalog lines show their exact excerpts and verification dates. Technical details retain source-line IDs and supported-claim IDs. The catalog does not carry claim text, so this implementation does not invent claim descriptions or read postcommit artifacts. Existing HTTPS-only network link hooks and offline-status text are preserved. Source titles remain the page heading; compact inline citations retain their existing presentation.

Validation: site TypeScript and 19 targeted tests passed, including two new citation cases for retained metadata/escaping and absent/invalid/non-HTTPS link behavior. Production build and artifact checks passed (1,352 documents, 224 item-scoped artifacts, 291 byte-identical delivery assets, 61 safe shell URLs). Logs: `/tmp/nyc-citation-final-tests.log`, `/tmp/nyc-citation-metadata-build.log`. Desktop/compact visual comparison and browser network-state checks of these changed source detail pages remain open; this is not full SourceCitation visual acceptance.

Source citation browser follow-up: `source-citation-current/REVIEW.md` records 7 browser passes and 2 explicit service-worker skips, including the corrected semantic Supported scope test locator. `source-citation-measure/REVIEW.md` records the observed compact indentation fix, fresh viewport inspection and six repeated three-engine source-page passes. Current source record quotes use the card's inline width. Full matching-reference visual acceptance and other citation families remain open.
